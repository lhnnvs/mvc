...
<body class="d-flex flex-column bg-dark vh-100">
    <div class="container d-flex flex-column flex-fill text-white fs-4 pt-4 px-4" style="max-width: 480px; background-image: url('../assets/images/bg.jpg'); background-size: cover;">
        ...
        <div class="d-flex flex-fill align-items-center justify-content-center text-center">
            <form method="POST" action="<?= ROOT ?>/mobile/home" class="d-flex flex-column align-items-center">
                ...
                <input type="hidden" name="access" id="accessValue" />
                <input type="hidden" name="id" value="<?= isset($_GET['id']) ? $_GET['id'] : 0 ?>" />
                <button type="submit" class="btn btn-light fw-bold fs-4 w-75" id="confirmButton">Confirm</button>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function confirmPin() {
            var pinCode = "<?= isset($user->pincode) ? $user->pincode : '' ?>";
            if (pin.length === 6 && pin === pinCode) {
                const params = new URLSearchParams(window.location.search);
                const action = params.get('action');
                if (action === 'unlock') {
                    alert("Locker unlocked.");
                    document.getElementById('accessValue').value = "Locked";
                } else if (action === 'lock') {
                    document.getElementById('accessValue').value = "Unlocked";
                }
            } else {
                if (pin.length < 6) {
